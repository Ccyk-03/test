#!/usr/bin/env python3
"""macOS 版配置导入脚本（可放在任意目录运行，仅用 Python 标准库）。

用法（在任意目录）：
    python3 apply_config_mac.py <配置文件>      # 指定文件
    python3 apply_config_mac.py                 # 默认读同目录 config_import.txt

会自动定位（均在用户目录，不随 .app 一起被拷贝）：
    ~/Library/Application Support/抽卡师的魔法/config.json
    ~/Library/Application Support/抽卡师的魔法/app.db
